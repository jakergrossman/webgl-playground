 ...
 *
 * @param resolution Number the value of N
 * @param offsets Object a 2-dimension list:
 *                         offsets[0]: A 2 dimension lists representing the lower and upper X bound
 *                         offsets[1]: A 2 dimension lists representing the lower and upper Y bound
 * @param perm Object the permutation to use for the noise algorithm
 */
export function mat2(resolution, offsets=[[0,16],[0, 16]], perm=kenPerlinPerm) {
   let xRange = linSpace(offsets[0][0], offsets[0][1], resolution),
       yRange = linSpace(offsets[1][0], offsets[1][1], resolution);

   let noise3d = perlin(xRange, yRange, [0], perm);
   let noise2d = createNDArray([resolution, resolution]);
   for (let i = 0; i < resolution; i++) {
      for (let j = 0; j < resolution; j++) {
         noise2d[i][j] = noise3d[i][j][0];
         console.log(noise2d[i][j]);
      }
   }
