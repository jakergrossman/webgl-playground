...
import { lerp, createNDArray } from "../misc.js";
...
export function mat3(xs, ys, zs, perm=kenPerlinPerm) {
   ...
   let data = createNDArray([zs, ys, xs].map(n=>n.length), 0);

   for (let i = 0; i < zs.length; i++) {
      for (let j = 0; j < ys.length; j++) {
         for (let k = 0; k < xs.length; k++) {
            data[i][j][k] = point(xs[k], ys[j], zs[i], perm);
         }
      }
   }
 ...
 * @param ys Object list of y dimension points
 * @param zs Object list of z dimension points
 * @param octaves Number octave count
 * @param persistence Number weight of each octave relative to the last
 * @param lacunarity Number amount of detail added per octave (dimension scaling amount)
 * @param perm Object permutation for perlin algorithm.
 *                    If "falsy", this function returns null.
 *                    Otherwise, it is extended to size >= 512 to avoid
 *                    modulo in algorithm
 */
export function mat3Octave(
   xs,
   ys,
   zs,
   octaves=2,
   ...
   perm=kenPerlinPerm)
{
   if (octaves === 1) return mat3(xs,ys,zs,perm);
   ...
   // max value to scale back to [-1,1] range
   let maxValue = 1;
   for (let o = 1; o < octaves; o++) {
      maxValue += Math.pow(persistence, octaves);
   }
   for (let i = 0; i < zs.length; i++) {
      for (let j = 0; j < ys.length; j++) {
         for (let k = 0; k < xs.length; k++) {
            ...
            for (let o = 0; o < octaves; o++) {
               data[i][j][k] += intensity * point(xs[k]*scale, ys[j]*scale, zs[i]*scale, perm);
...
export function mat2(xs, ys, z=0, perm=kenPerlinPerm) {
   ...
   let noise2d = createNDArray([ys, xs].map(n=>n.length));
   for (let i = 0; i < ys.length; i++) {
      for (let j = 0; j < xs.length; j++) {
         noise2d[i][j] = noise3d[0][i][j];
      }
   }
...
export function mat2Octave(
   ...
   perm=kenPerlinPerm)
{
   if (octaves === 1) return mat2(xs,ys,z,perm);

   let noise3d = mat3Octave(xs, ys, [z], octaves, persistence, lacunarity, perm);
   let noise2d = createNDArray([ys, xs].map(n=>n.length));
   for (let i = 0; i < ys.length; i++) {
      for (let j = 0; j < xs.length; j++) {
         noise2d[i][j] = noise3d[0][i][j];
