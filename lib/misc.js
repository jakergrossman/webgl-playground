...
export async function fetchText(uri) {
    ...
    if (response.status !== 200) {
        throw new URIError("Could not load resource \"" + uri + "\"");
    }

    return await response.text();
}

export function fetchTextSync(uri) {
...
export function doubleToSinglePrecisions(n) {
    const highPart = Math.fround(n);
    const lowPart = n - highPart;

    return [highPart, lowPart];
}

export function getQueryVariables(...names) {
    let result = {};
    const query = window.location.search.slice(1);
    const vars = query.split("&");
    for (const v of vars) {
        const pair = v.split("=");
        if (names.length === 0 || names.includes(decodeURIComponent(pair[0]))) {
...
export function createQueryURI(options, baseURI) {
    if (!baseURI) {
        let qPos = window.location.toString().indexOf("?");
        baseURI = window.location.toString();
        if (qPos != -1) baseURI = baseURI.slice(0, qPos);
    }

    const params = Object.entries(options);
    return "?" + params.map(n=>n.join("=")).join("&");
}
