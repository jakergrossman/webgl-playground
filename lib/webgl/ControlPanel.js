...
export class ControlPanel {
    ...
    /**
     * Return the current values of control elements as a JS object
     */
    values() {
        const controls = Object.entries(this).filter(n=>this.hasOwnProperty(n[0]) && n[0].endsWith("Control"));
        const values = {};
        for (const c of controls) {
            values[c[0].replace("Control", "")] = parseFloat(c[1].value);
        }
        return values;
    }
}
