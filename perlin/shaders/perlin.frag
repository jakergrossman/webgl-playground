...
vec3 grad(vec3 p, float gradSeed) {
    return normalize(vec3(rand(vec2(p.x * gradSeed, p.y)),
                          rand(gradSeed * vec2(p.x, p.y) * p.z),
                          rand(vec2(p.x, p.y*gradSeed) + p.z)));
}
...
float perlin(vec3 co, float pSeed) {
    vec3 point = vec3(co.xy / scale, zDepth);
    ...
    vec3 g000 = grad(c000, pSeed);
    vec3 g001 = grad(c001, pSeed);
    vec3 g010 = grad(c010, pSeed);
    vec3 g011 = grad(c011, pSeed);
    vec3 g100 = grad(c100, pSeed);
    vec3 g101 = grad(c101, pSeed);
    vec3 g110 = grad(c110, pSeed);
    vec3 g111 = grad(c111, pSeed);
